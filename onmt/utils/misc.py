...
def split_corpus(path, shard_size, default=None, sentences=None):
    ...
    if path is not None:
        ...
        return repeat(default) if sentences is None else _split_corpus_with_list_type(sentences, shard_size)
...
def _split_corpus_with_list_type(sentences, shard_size):
    if shard_size <= 0:
        yield sentences
    else:
        while True:
            shard = list(islice(sentences, shard_size))
